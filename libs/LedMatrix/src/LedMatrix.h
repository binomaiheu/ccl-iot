...
public:
    ...
    /**
     * Draw the currently set bytes (need to be an array of 8 bytes).
     */
    void tekenMatrix(boolean data[8][8]);
...
private:
    ...
    bool deviceOrientation = 0; // 1 is vertical - 0 is horizontal
    bool flip = 0; //
    int8_t _sck;
    int8_t _miso;
    int8_t _mosi;
    void calculateTextAlignmentOffset();
    byte getByte(boolean boolArray[8]);
};
