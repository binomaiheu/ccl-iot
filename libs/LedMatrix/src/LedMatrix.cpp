    ...
    if (deviceOrientation == 0) {
        ...
        for (col = 0; col < myNumberOfDevices * 8; col++) {
            for(byte bits = 0; bits < 8; bits++) {
                xcols[col] |= ((cols[bits + 8*(col/8)] & (index << (col%8))) ? (B10000000 >> bits) : 0);
            }
...
void LedMatrix::tekenMatrix(boolean values[8][8]) {
    deviceOrientation = 0;
    boolean boolArray[8];
    for (size_t i = 0; i < 8; i++) {
        for (size_t j = 0; j < 8; j++) {
            boolArray[j] = values[j][i];
        }
        setColumn(i, getByte(boolArray));
    }
}
...
byte LedMatrix::getByte(boolean boolArray[8]) {
  byte result = 0; 
  for (int i = 0; i < 8; i++) {
    if (boolArray[i]) {
      result = result | (1 << i);
